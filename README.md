# Projekt L – Rozwiązania i tłumaczenia list z podstaw programowania

Ten projekt zawiera rozwiązania ćwiczeń z **podstaw programowania**.  

## Struktura repozytorium

- `story/solutions` – opis problemów i kontekstu ćwiczeń

## Lista ćwiczeń

- **Lista 2**
    - [Link do ćwiczeń](https://andrzejewski.wppt.pwr.edu.pl/assets/pp_opt_l02_26.pdf)
    - [Link do rozwiazań](https://github.com/MaksymilianMazurkiewicz/ft_basics_of_programming/tree/story/solutions/second_list)
- **Lista 3**
    - [Link do ćwiczeń](https://andrzejewski.wppt.pwr.edu.pl/assets/pp_opt_l03_26.pdf)
    - [Link do rozwiazań](https://github.com/MaksymilianMazurkiewicz/ft_basics_of_programming/tree/story/solutions/third_list)  
- **Lista 4**
    - [Link do ćwiczeń](https://andrzejewski.wppt.pwr.edu.pl/assets/pp_opt_l04_26.pdf)
    - [Link do rozwiazań](https://github.com/MaksymilianMazurkiewicz/ft_basics_of_programming/tree/story/solutions/fourth_list)  
- **Lista 5**
    - [Link do ćwiczeń](https://andrzejewski.wppt.pwr.edu.pl/assets/pp_opt_l05_26.pdf)  
    - [Link do rozwiazań](https://github.com/MaksymilianMazurkiewicz/ft_basics_of_programming/tree/story/solutions/fifth_list)
  
## Jak korzystać z repozytorium
1. Przejdź do odpowiedniego brancha z rozwiązaniami list 
2. W `solutions` znajdziesz pliki z gotowymi rozwiązaniami zadań.

## Wymagania

- Interpreter do Pytyhon 3.12

---

> Ten projekt jest przeznaczony do celów edukacyjnych i nauki programowania. 
